...
if (isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true) {
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		$tmpCpnId = intval($_POST['iCpnId']);
		$tmpQntty = intval($_POST['iQntty']);
		$tmpPrc = intval($_POST['iPrc']);
		$tmpDscnt = intval($_POST['iDscnt']);
		...
		if (isset($_POST['btnSave'])) {
			if (isset($tmpId)) {
				if ($tmpId > 0) {
					if (isSaved("cioId", "CouponInOut", $tmpId)) {
						if (isSaved("bhSub", "BillHeader", $_SESSION['Sub'], " AND bhKnd=11 AND bhId > " . $_SESSION['iDly'])) {
							...
							$vl = array($tmpCpnId, $tmpQntty, $tmpPrc, $tmpDscnt, $tmpNts, $tmpId);
					...
					$vl = array( $_SESSION['iDly'], $tmpCpnId, $tmpQntty, $tmpPrc, $tmpDscnt , $tmpNts) ;
	...
	elseif ($_SERVER['REQUEST_METHOD'] == "GET") {
		if (isset($_GET['iDly'])) {
			...
			$_SESSION['iDly'] = 1;
			$report = "لم يتم تحديد الفاتورة بشكل صحيح" ;
		}
		//------------------------------------------
		if (isset($_GET['srt'])) {
...
?>
	...
	<div class="row" id="row">
		...
		<form action="<?php echo ($_SERVER['PHP_SELF']); ?>" method="post" name="iFrm" onsubmit="return isValidForm()">
			...
			<input class="" id="iDly" name="iDly" type="hidden" value="<?php echo($_SESSION['iDly']) ?>" />
				...
				<select class="swing" name="iCpnId" id="iCpnId" placeholder="اسم البون">
					<?php
						...
						if ($row['cId'] == $tmpCpnId) {
							...
							echo ("<option value='" . $row['cId'] . "'". $sl . ">" . $row['cNm'] . "</option> ");
						}
					?>''
				</sele'ct><label for="iCpnId"> اسم البون </label>
			</span>
			...
			<select class="hiddenCol" name="tmp" id="tmp" >
				<?php
					...
					foreach ($result as $row) {
						...
						echo ("<option value='" . $row['cId']. "'". $sl . ">" . $row['cByPrc']. "</option> ");
					}
				?>
			</select>
			<span>
				<input class="swing" id="iQntty" name="iQntty" type="number" maxlength="10" autocomplete="off" placeholder="الكمية أو العدد" value="<?php echo($tmpQntty) ?>" /><label for="iQntty">الكمية</label>
			</span>
			<span>
				<input class="swing" id="iPrc" name="iPrc" type="number" maxlength="10" autocomplete="off" placeholder="قيمة البون" value="<?php echo($tmpPrc) ?>" required /><label for="iPrc">القيمة</label>
			</span>
			<span>
				<input class="swing" id="iDscnt" name="iDscnt" type="number" maxlength="10" autocomplete="off" placeholder="قيمة الخصم" value="<?php echo($tmpDscnt) ?>" /><label for="iDscnt">الخصم</label>
			</span>
			<span>
				<input class="swing" id="iNts" name="iNts" type="text" maxlength="99" autocomplete="off" placeholder="ملاحظات" value="<?php echo($tmpNts) ?>" /><label for="iNts">ملاحظات</label>
			</span>
